...
const EditarActividad  = ({ route }) => {
  ...
  const [duracion, setDuracion] = useState(data.duracion);
  const [series, setSeries] = useState(data.series);
  const [repeticiones, setRepeticiones] = useState(data.repeticiones);
  ...
  const handleGuardar = async () => {
    ...
    await axios.put(`http://127.0.0.1:8000/api/actividades/${id}/editar/`, data, config)
      .then(response => {
